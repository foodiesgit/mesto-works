...
class ThreeDPayController extends Controller
{
    ...
    public function index(Request $request)
    {
      ...
      if(session('doc'))
      {
        $document = Document::where('KabulBelgesiId' , session('doc'))->get()->first();
        $document->BasvuruDurumId = 3 ;
        $document->AktifMi = 1 ;
        $document->save();

        $yeni = new D();
        $yeni->ReferansKodu = "TEST";
        $yeni->KabulBelgesiId = session('doc');
        $yeni->Tutar = session('odeme');
        $yeni->OdemeZamani = date('Y-m-d H:i:s');
        $yeni->OdemeBasariDurumu = 1 ;
        $yeni->OdemeKodu = 1;
        $yeni->IsletenKullaniciId = session('Id');
        $yeni->Mod = "WEB";
        if($yeni->save())
        {
          $request->session()->flash('message', 'Ödeme Gerçekleştirildi');
          return redirect('/documents');
        }
        else
        {
          die("Kayıt yazma hatası oluştu");
        }
      }
