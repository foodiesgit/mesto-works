...
Route::middleware('checkSession')->post('/admin/uploadimage/', function(Request $req){
   $fileName = $req->file->getClientOriginalName();
    DB::table('images')->insert([
        ...
        'imgname'=>'uploads/'.$req->maincategory.'/'.$req->subcategory.'/'.$fileName
    ]);
...
Route::middleware('checkSession')->post('/admin/getimages', function(Request $req){
    return $data['imgname'] = DB::table('images')->where('subcategory', $req->subcategory)->get(['imgname']);
});
Route::middleware('checkSession')->post('/admin/deleteimage', function(Request $req){
    DB::table('images')->where('imgname', $req->imgname, 1)->delete();
    File::delete($req->imgname);
    return '200';
});
