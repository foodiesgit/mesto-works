          ...
          <details :open="isLeagueOpen" class="odd-details cp" v-for="(list, index) in leagues" :key="index">
            ...
            <div class="basket-list" v-for="(sublist, index) in filterBy(list, searchBox)" :key="index">
              ...
              <div class="basket-right-list">
                <div class="ottli-bas ottli-bas-list rate-text-basket" @click="addSlip($event, 'Evsahibi', sublist, 'Maç Sonu')">{{sublist.home | convertOdd($store.state.newrate)}}</div>
                <div class="ottli-bas ottli-bas-list rate-text-basket" @click="addSlip($event, 'Deplasman', sublist, 'Maç Sonu')">{{sublist.away | convertOdd($store.state.newrate)}}</div>
                <div class="ottli-bas ottli-bas-list rate-text-basket" @click="addSlip($event, sublist.under, sublist, 'Toplam Sayı Alt/Üst')">
...
export default {
  ...
  methods: {
    async getLeagues(){
      ...
      await axios.get('/api/home/basketfixture').then((result)=>{
        if (result.data) {
          ...
          const favtemp = final.filter(item => item.RegionId === 31 || item.RegionId === 37 || item.RegionId === 14 || item.RegionId === 36 || item.RegionId === 17 || item.RegionId === 16 || item.RegionId === 20)
    ...
    searchFilter () {
      const searchArray = this.search.filter(item => item.HomeTeam.toLowerCase().includes(this.searchBox.toLowerCase()) || item.AwayTeam.toLowerCase().includes(this.searchBox.toLowerCase()) || item.fixtureid.toString().slice(-3).includes(this.searchBox))
